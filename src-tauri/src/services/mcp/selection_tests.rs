...
fn fixture(initialized: bool) -> AppState {
    if initialized {
...
fn native_entries(app: &AppType, path: &std::path::Path) -> Value {
...
fn observed_files() -> Vec<(PathBuf, Option<Vec<u8>>)> {
    paths()
        .into_iter()
        .map(|(_, path)| {
...
fn assert_files(before: &[(PathBuf, Option<Vec<u8>>)]) {
