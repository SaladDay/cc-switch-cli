//! One standalone toggle owns its catalog selection and retained native recovery.
...
impl McpService {
    pub(super) fn toggle_coordinated<N: NativeToggle>(
        state: &AppState,
        id: &str,
        app: AppType,
        enabled: bool,
        observe: impl FnOnce() -> Result<N, AppError>,
    ) -> Result<(), AppError> {
        ...
        let sync_live = crate::sync_policy::should_sync_live(&app);
        ...
        let mut native = if sync_live { Some(observe()?) } else { None };
        let result = (|| {
            if let Some(native) = native.as_mut() {
                let snapshot =
                    native.apply(id, &server.server, enabled, previous_snapshot.as_ref())?;
