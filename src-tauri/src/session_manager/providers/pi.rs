...
pub fn session_discovery_error() -> Option<String> {
    match session_discovery() {
        PiSessionDiscovery::Available => None,
        PiSessionDiscovery::RequiresProjectContext { configured_path } => Some(format!(
            "Pi sessionDir '{configured_path}' requires a project cwd and cannot be globally enumerated"
        )),
        PiSessionDiscovery::Unavailable { reason } => Some(reason),
    }
}
...
#[allow(
    dead_code,
    reason = "retained as the upstream-compatible eager snapshot adapter"
)]
...
pub(crate) fn stream_sessions_cancellable(
    ...
    stream_sessions_from_resolution_cancellable(
        store,
        force,
...
fn stream_sessions_from_resolution_cancellable(
    store: Option<&ScanCacheStore>,
    force: bool,
    resolution: SessionRootResolution,
    on_session: &mut dyn FnMut(SessionMeta) -> ControlFlow<()>,
    ...
    let (root, layout) = match resolution {
        SessionRootResolution::Available { root, layout } => (root, layout),
        SessionRootResolution::RequiresProjectContext { configured_path } => {
            log::warn!(
                "Pi sessionDir '{configured_path}' requires a project cwd and cannot be globally enumerated"
            );
            return Ok(cache::StreamScanStats::default());
        }
        SessionRootResolution::Unavailable { reason } => {
            log::warn!("Pi session discovery unavailable: {reason}");
            return Ok(cache::StreamScanStats::default());
    ...
    cache::stream_file_provider_cancellable(
        ...
        |path| {
            ...
            let parsed = match parse_session_cancellable(path, is_cancelled) {
        ...
        move |on_target, cancel| {
            visit_session_targets_cancellable(&root, layout, on_target, cancel)
        },
        on_session,
        is_cancelled,
    )
...
fn visit_session_targets_cancellable(
    root: &Path,
    layout: SessionLayout,
    on_target: &mut dyn FnMut(cache::FileScanTarget) -> Result<(), cache::StreamScanStop>,
    is_cancelled: &(dyn Fn() -> bool + Sync),
    ...
    let entries = match fs::read_dir(root) {
        ...
        Err(error) => {
            log::warn!("Pi session walk failed at {}: {error}", root.display());
    ...
    for entry in entries {
        ...
        let entry = entry.map_err(|error| {
            log::warn!(
                "Pi session directory entry failed at {}: {error}",
                root.display()
            );
        ...
        let file_type = entry.file_type().map_err(|error| {
            log::warn!(
                "Pi session file type failed at {}: {error}",
                entry.path().display()
            );
            cache::StreamScanStop::Incomplete
        })?;
        match layout {
            SessionLayout::Flat if file_type.is_file() => {
                visit_session_target(entry.path(), on_target)?;
            }
            SessionLayout::ProjectDirectories if file_type.is_dir() => {
                let project_path = entry.path();
                let project_entries = fs::read_dir(&project_path).map_err(|error| {
                    log::warn!(
                        "Pi project session walk failed at {}: {error}",
                ...
                for project_entry in project_entries {
                    ...
                    let project_entry = project_entry.map_err(|error| {
                        log::warn!(
                            "Pi project session entry failed at {}: {error}",
                            project_path.display()
                        );
                        cache::StreamScanStop::Incomplete
                    })?;
                    let project_file_type = project_entry.file_type().map_err(|error| {
                        log::warn!(
                            "Pi project session file type failed at {}: {error}",
                            project_entry.path().display()
                        );
                        cache::StreamScanStop::Incomplete
                    })?;
                    if project_file_type.is_file() {
                        visit_session_target(project_entry.path(), on_target)?;
                    }
                }
            }
...
fn visit_session_target(
    ...
    let target = cache::stat_target_strict(&path).map_err(|error| {
        log::warn!("Pi session stat failed at {}: {error}", path.display());
        cache::StreamScanStop::Incomplete
    })?;
    if let Some(target) = target.filter(|target| target.size <= MAX_SESSION_BYTES as i64) {
        on_target(target)?;
    }
    Ok(())
}
...
pub(crate) fn load_messages_cancellable(
    ...
    match resolve_session_root() {
        SessionRootResolution::Available { root, layout, .. } => {
            load_messages_with_layout_cancellable(&root, path, layout, is_cancelled)
        }
        SessionRootResolution::RequiresProjectContext { .. } => {
            Err("Relative Pi sessionDir cannot be globally resolved".to_string())
        }
...
fn load_messages_with_layout_cancellable(
    root: &Path,
    path: &Path,
    layout: SessionLayout,
    is_cancelled: &(dyn Fn() -> bool + Sync),
    ...
    let (_, source) = validate_source_under_root(root, path, layout)?;
    let tree = read_tree_cancellable(&source, is_cancelled)?
        .ok_or_else(|| "Session message preview was cancelled".to_string())?;
    read_active_message_batch(&source, &tree, is_cancelled)
}
...
pub(crate) fn search_session_cancellable(
    ...
    let (root, layout) = match resolve_session_root() {
        SessionRootResolution::Available { root, layout, .. } => (root, layout),
        SessionRootResolution::RequiresProjectContext { .. }
        | SessionRootResolution::Unavailable { .. } => return None,
    };
    search_session_with_layout_cancellable(meta, &root, path, layout, needle, is_cancelled)
}

fn search_session_with_layout_cancellable(
    meta: &SessionMeta,
    root: &Path,
    path: &Path,
    layout: SessionLayout,
    ...
    let (_, source) = validate_source_under_root(root, path, layout).ok()?;
    let tree = read_tree_cancellable(&source, is_cancelled).ok()??;
    let mut snippets = Vec::new();
    const MAX_SNIPPETS: usize = 5;
    visit_active_messages_cancellable(&source, &tree, is_cancelled, &mut |message| {
        match build_snippet_cancellable(&message.content, needle, is_cancelled) {
            Ok(Some(snippet)) => {
...
fn parse_session(path: &Path) -> Result<SessionMeta, String> {
    parse_session_cancellable(path, &|| false)?
        .ok_or_else(|| "Pi session parse was cancelled".to_string())
}
...
fn parse_session_cancellable(
    ...
    let SessionTree {
        header, summary, ..
    } = match read_tree_cancellable(&source, is_cancelled)? {
        Some(tree) => tree,
        None => return Ok(None),
    };
...
fn read_tree(path: &Path) -> Result<SessionTree, String> {
    read_tree_cancellable(path, &|| false)?
        .ok_or_else(|| "Pi session tree read was cancelled".to_string())
}

fn read_tree_cancellable(
    path: &Path,
    is_cancelled: &(dyn Fn() -> bool + Sync),
) -> Result<Option<SessionTree>, String> {
    if is_cancelled() {
        return Ok(None);
    }
...
fn read_active_messages(path: &Path, tree: &SessionTree) -> Result<Vec<SessionMessage>, String> {
    ...
    visit_active_messages_cancellable(path, tree, &|| false, &mut |message| {
        messages.push(message);
        ControlFlow::Continue(())
    })?
    .ok_or_else(|| "Pi session message read was cancelled".to_string())?;
    Ok(messages)
}

fn read_active_message_batch(
    path: &Path,
    tree: &SessionTree,
    is_cancelled: &(dyn Fn() -> bool + Sync),
) -> Result<SessionMessageBatch, String> {
    let mut batch = SessionMessageBatchBuilder::new();
    visit_active_messages_cancellable(path, tree, is_cancelled, &mut |message| {
        batch.push(message)
    })?
    .ok_or_else(|| "Session message preview was cancelled".to_string())?;
    Ok(batch.finish())
...
fn visit_active_messages_cancellable(
    path: &Path,
    tree: &SessionTree,
    is_cancelled: &(dyn Fn() -> bool + Sync),
    on_message: &mut dyn FnMut(SessionMessage) -> ControlFlow<()>,
    ...
    for line in reader.lines() {
        ...
        let message = match value.get("type").and_then(Value::as_str) {
            ...
            Some("compaction") | Some("branch_summary") => system_message(
                value
                    .get("summary")
                    .and_then(Value::as_str)
                    .unwrap_or_default(),
...
fn system_message(content: &str, ts: Option<i64>) -> Option<SessionMessage> {
    (!content.trim().is_empty()).then(|| SessionMessage {
...
mod tests {
    ...
    #[test]
    fn message_preview_is_bounded_and_marked_truncated() {
        let temp = tempfile::tempdir().expect("tempdir");
        let root = temp.path().join("sessions");
        fs::create_dir_all(&root).expect("root");
        ...
        let mut body =
            "{\"type\":\"session\",\"version\":3,\"id\":\"session-1\",\"cwd\":\"/work\"}\n"
                .to_string();
        for index in 0..=crate::session_manager::SESSION_MESSAGE_PREVIEW_MAX_MESSAGES {
            ...
            body.push_str(&format!(
                "{{\"type\":\"message\",\"id\":\"message-{index}\",\"parentId\":{parent},\"message\":{{\"role\":\"assistant\",\"content\":\"message {index}\"}}}}\n"
            ));
        }
        fs::write(&path, body).expect("session");

        let batch =
            load_messages_with_layout_cancellable(&root, &path, SessionLayout::Flat, &|| false)
    ...
    #[test]
    fn message_preview_honors_cancellation() {
        let temp = tempfile::tempdir().expect("tempdir");
        let root = temp.path().join("sessions");
        fs::create_dir_all(&root).expect("root");
        let path = root.join("cancel.jsonl");
        write_session_header(&path, "session-1");

        let error =
            load_messages_with_layout_cancellable(&root, &path, SessionLayout::Flat, &|| true)
                .expect_err("cancelled preview");
        assert!(error.contains("cancelled"));
    }

    #[test]
    fn deep_search_reads_the_active_pi_branch() {
        let temp = tempfile::tempdir().expect("tempdir");
        let root = temp.path().join("sessions");
        fs::create_dir_all(&root).expect("root");
        let path = root.join("search.jsonl");
        fs::write(
            &path,
            "{\"type\":\"session\",\"version\":3,\"id\":\"session-1\",\"cwd\":\"/work\"}\n\
             {\"type\":\"message\",\"id\":\"root\",\"parentId\":null,\"message\":{\"role\":\"user\",\"content\":\"start\"}}\n\
             {\"type\":\"message\",\"id\":\"match\",\"parentId\":\"root\",\"message\":{\"role\":\"assistant\",\"content\":\"hidden pi needle\"}}\n\
             {\"type\":\"message\",\"id\":\"done\",\"parentId\":\"match\",\"message\":{\"role\":\"assistant\",\"content\":\"done\"}}\n",
        )
        .expect("session");
        let meta = parse_session(&path).expect("metadata");

        let hit = search_session_with_layout_cancellable(
            &meta,
            &root,
            &path,
            SessionLayout::Flat,
        ...
        assert_eq!(hit.snippets[0].role, "assistant");
        assert!(hit.snippets[0].snippet.contains("pi needle"));
    }
    ...
    #[test]
    fn streaming_scan_honors_cancellation_during_discovery() {
        ...
        for index in 0..200 {
            write_session_header(
                &root.join(format!("session-{index:03}.jsonl")),
                &format!("session-{index:03}"),
            );
        }
        let checks = AtomicUsize::new(0);
        let emitted = AtomicUsize::new(0);
        let is_cancelled = || checks.fetch_add(1, Ordering::AcqRel) >= 20;
        let mut sink = |_: SessionMeta| {
            emitted.fetch_add(1, Ordering::AcqRel);
            ControlFlow::Continue(())
        };

        let result = stream_sessions_from_resolution_cancellable(
            None,
            false,
        ...
        assert_eq!(emitted.load(Ordering::Acquire), 0);
        assert!(checks.load(Ordering::Acquire) < 100);
    }
