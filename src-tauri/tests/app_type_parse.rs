...
#[test]
fn app_parser_accepts_only_cli_canonical_ids() {
    for value in ["open-code", "open_claw", "claudedesk", "grok"] {
        assert!(
            AppType::from_str(value).is_err(),
            "noncanonical Core alias must not widen the CLI input contract: {value}"
        );
    }
}

#[test]
fn display_names_follow_core_registry_metadata() {
    for app in AppType::all() {
        let core = CoreAppType::from(app.clone());
        assert_eq!(
            app.display_name(),
            builtin_app_registry().for_app(&core).display_name()
        );
    }
}
